...
import { KeyTunerWasm, initKeyTunerWasm } from "../../wasm_artifacts/key-tuner-wasm";
...
window.addEventListener("DOMContentLoaded", () => {
    // DOMイベントのセットアップ
    setupDom(initKeyTunerWasm);
});

/**
 * DOM要素のセットアップとイベントハンドラの登録
 * @param {() => KeyTunerWasm} initWasm - WASMモジュールを初期化する関数
 */
async function setupDom(initWasm: () => Promise<KeyTunerWasm>): Promise<void> {
    // DOM要素とエラーハンドラを初期化
